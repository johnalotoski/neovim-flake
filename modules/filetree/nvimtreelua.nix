...
let
  ...
  toLua = level: o:
    let type = builtins.typeOf o;
    in if type == "bool" then
      lib.boolToString o
    else if type == "string" then
      ''"${o}"''
    else if type == "list" then
      "{" + (lib.concatStringsSep ", " (map toLua o)) + "}"
      ...
      (lib.mapAttrsToList (k: v: "${indent level}${k} = ${toLua (level + 2) v}")
        o)}
      ${indent (level - 2)}}'' else
      throw "Cannot translate unknown type '${type}' to Lua";
...
in {
      ...
      type = types.submodule {
        options = {
          ...
          open_on_setup = mkOption {
            default = false;
            description =
              "will automatically open the tree when running setup if current buffer is a directory, is empty or is unnamed.";
            type = types.bool;
          };

          ignore_ft_on_setup = mkOption {
            description =
              "list of filetypes that will make open_on_setup not open. You can use this option if you don't want the tree to open in some scenarios (eg using vim startify).";
            ...
            type = types.submodule {
              options = {
                ...
                ignore_list = mkOption {
                  description =
                    "list of buffer names and filetypes that will not update the root dir of the tree if the file isn't found under the current root directory. Only relevant when |update_focused_file.update_cwd| is `true` and |update_focused_file.enable| is `true`.";
                  type = types.listOf types.str;
                  default = [ ];
                };
              };
          ...
          system_open = mkOption {
            ...
            type = types.submodule {
              options = {
                ...
                cmd = mkOption {
                  description =
                    "the command to run, leaving nil should work but useful if you want to override the default command with another one.";
          ...
          git = mkOption {
            ...
            type = types.submodule {
              options = {
                ...
                timeout = mkOption {
                  description =
                    "kills the git process after some time (in ms) if it takes too long";
          ...
          view = mkOption {
            ...
            type = types.submodule {
              options = {
                ...
                width = mkOption {
                  description =
                    "width of the window, can be either a `%` string or a number representing columns. Only works with |view.side| `left` or `right`";
                ...
                height = mkOption {
                  description =
                    "height of the window, can be either a `%` string or a number representing rows. Only works with |view.side| `top` or `bottom`";
                  type = types.oneOf [ types.str types.ints.positive ];
                  default = 30;
                };
                ...
                side = mkOption {
                  description =
                    "side of the tree, can be one of 'left' | 'right' | 'bottom' | 'top' Note that bottom/top are not working correctly yet.";
                  type = types.enum [ "left" "right" "bottom" "top" ];
                  default = "left";
                };

                auto_resize = mkOption {
                  description = "auto resize the tree after opening a file";
                  type = types.bool;
                  default = false;
                };
                  ...
                  type = types.submodule {
                    options = {
                      custom_only = mkOption {
                        description =
                          "will use only the provided user mappings and not the default otherwise, extends the default mappings with the provided user mappings";
                        type = types.bool;
                        default = false;
                      };
  ...
  config = mkIf cfg.enable (let mkVimBool = val: if val then 1 else 0;
  in {
